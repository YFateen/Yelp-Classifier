...
class YelpClassifier(object):
    ...
    def calculate_likelihoods(self, train_rdd):
        ...
        class_likelihoods = train_rdd \
                            ...
                            .aggregateByKey(dict(), self.combine_probability_tables, self.combine_probability_tables, 3) #changed reducedByKey and aggregateByKey, not sure about parameters of aggregate 
    ...
    def calculate_num_reviews_and_words_per_num_stars(self, train_rdd):
        ...
        num_reviews_words_per_num_stars = train_rdd \
                                          .flatMap(self.review_to_num_stars_num_words) \
    ...
    # 3. [((num_stars, word_in_review), num_reviews_with_word_and_stars)] --> [(num_stars, {word : probability_in_review_of_num_stars})]
    def counts_to_probabilities(self, num_stars_and_word_counts):   #changed, assume access to (key,value) same as arr 
        # raise NotImplementedError()
        key, value = num_stars_and_word_counts
        num_stars, word = key
        prob = self.compute_likelihood(int(value), int(num_stars))
        return (num_stars, {word: prob})
    ...
    # 1. (review_id, num_stars, review_text_as_string) --> [(num_stars, (1, num_words))]
    @staticmethod
    def review_to_num_stars_num_words(review): #working on this 
        # raise NotImplementedError()
        words = review[2].split(" ")
        length = len(words)
        return [(review[1],(1, length))]

    # Combines the values of two pairs outputted by review_to_num_stars_num_words
    # 2. [(num_stars, (1, num_words))] --> [(num_stars, (num_reviews_of_num_stars, num_words_total_of_num_stars))]
    @staticmethod
    def add_review_and_word_counts(count1, count2): #workign on this 
        # raise NotImplementedError()
